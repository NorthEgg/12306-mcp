...
import { API_BASE, CITY_CODES, CITY_STATIONS, LCQUERY_PATH, NAME_STATIONS, SEARCH_API_BASE, STATIONS, TIME_ZONE, } from "../constant/index.js";
import { server } from "./server.js";
import { TZDate } from "@date-fns/tz";
import { z } from "zod";
import { checkDate, filterTicketsInfo, formatCookies, formatInterlinesInfo, formatRouteStationsInfo, formatTicketsInfo, formatTicketsInfoCSV, getCookie, make12306Request, parseInterlinesInfo, parseRouteStationsInfo, parseTicketsData, parseTicketsInfo, } from "./index.js";
/** 获取当前日期 */
server.addTool({
...
server.addTool({
    ...
    async execute({ date, fromStation, toStation, trainFilterFlags, earliestStartTime, latestStartTime, sortFlag, sortReverse, limitedNum, format, }) {
        // 检查日期是否早于当前日期
        if (!checkDate(date)) {
        ...
        const queryResponse = await make12306Request(queryUrl, queryParams, {
...
server.addTool({
    ...
    async execute({ date, fromStation, toStation, middleStation, showWZ, trainFilterFlags, earliestStartTime, latestStartTime, sortFlag, sortReverse, limitedNum, format, }) {
        // 检查日期是否早于当前日期
        if (!checkDate(date)) {
            return "Error: The date cannot be earlier than today.";
        }
