...
import { API_BASE, DW_FLAGS, LCQUERY_INIT_URL, MISSING_STATIONS, SEAT_TYPES, STATION_DATA_KEYS, TICKET_DATA_KEYS, TIME_COMPARETOR, TIME_ZONE, TRAIN_FILTERS, WEB_URL, } from "../constant/index.js";
...
export function parseTicketsInfo(ticketsData, map) {
    const result = [];
...
export function extractPrices(yp_info, seat_discount_info, ticketData) {
...
export function extractDWFlags(dw_flag_str) {
...
export function filterTicketsInfo(ticketsInfo, trainFilterFlags, earliestStartTime = 0, latestStartTime = 24, sortFlag = "", sortReverse = false, limitedNum = 0) {
    let result;
    // FilterFlags过滤
...
export function formatTicketsInfoCSV(ticketsInfo) {
 ...
 * @returns 格式化后的票量描述
 */
export function formatTicketStatus(num) {
    // 检查是否为纯数字
    if (num.match(/^\d+$/)) {
...
export async function getLCQueryPath() {
...
export function parseInterlinesInfo(interlineData) {
    const result = [];
...
export function parseInterlinesTicketInfo(interlineTicketsData) {
    const result = [];
...
export function formatInterlinesInfo(interlinesInfo) {
    ...
    interlinesInfo.forEach((interlineInfo) => {
        ...
        result += `${interlineInfo.same_train ? "同车换乘" : interlineInfo.same_station ? "同站换乘" : "换站换乘"} | ${interlineInfo.wait_time} | ${interlineInfo.lishi}\n\n`;
...
export function formatRouteStationsInfo(routeStationsInfo) {
